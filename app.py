...
def _how_it_works():
    ...
    st.markdown(
        ...
        "- **Demand / burden (NFHS-5)** — per capability we average a few real district indicators "
        "(e.g. maternity: institutional-birth %, ≥4 ANC visits %, skilled-attendant %, anaemia %), each "
        "normalized 0–1 by direction (high- or low-is-worse); suppressed (`*`) values are dropped — never "
        "zeroed — and emergency/trauma have no NFHS proxy, so they're ranked on supply scarcity and labelled.\n"
